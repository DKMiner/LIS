...
def numeric_value():
    value_type = input("Is the value for this entry numeric and has a range?\n"
                       "1. Yes, enter the max and min values\n"
                       "2. No, leave blank\n"
                       "Enter the operation number: ")
    if value_type == "1":
        clean()
        minimum = num_ask("Enter the minimum value for this entry")
        maximum = num_ask("Enter the maximum value for this entry")
        while minimum > maximum:
            print("Minimum value cannot be bigger than maximum value!")
            maximum = num_ask("Enter the maximum value for this entry")
        ...
        if name in checklist:
            ...
            while True:
                ...
                answer2 = input(f'Do you want to add an entry for {name}?\n'
                                 "1. Add a new entry/Edit a previously entered entry\n"
                                 "0. save the test and return\n"
                                 "Input the operation number: ")
                if answer2 == "1":
                    clean()
                    item = input("Enter the entry name: ")
                    item_value = numeric_value()
                    while item_value == False:
                        item_value = numeric_value()
    ...
    if name not in names:
        ...
        for k,v in test_result.items():
            if k not in ("TID", "Name"):
                if v != "":
                    value = num_ask(f'enter the value for {k}: ')
                else:
                    value = input(f'enter the value for {k}: ')
                test_result[k] = value
        for person in test_results:
            if person[2] == x:
                person.append(test_result)
                                ...
                                if d["Name"] == x["Name"]:
                                    for k,v in d.items():
                                        if len(v.split(";")) > 1:
                                            ...
                                            maximum = m_list[1]
                                            if x[k] > maximum:
                                                result.append(f'{k}: {x[k]} (High)  *  min = {minimum} | max = {maximum}')
                                            elif x[k] < minimum:
                                                result.append(f'{k}: {x[k]} (Low)  *  min = {minimum} | max = {maximum}')
                                            else:
                                                result.append(f'{k}: {x[k]}  *  min = {minimum} | max = {maximum}')
                                        else:
                                            result.append(f'{k}: {x[k]}')
                            for n in result:
                                print(n)
                            send = input("-------------------------------------------\n"
                                         "Do you want to send this result to the patient?\n"
